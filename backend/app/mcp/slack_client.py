...
async def _resolve_channel_id(client: httpx.AsyncClient, name_or_id: str, create_if_missing: bool = True) -> str:
    ...
    # 1. List channels to find it (try public first, then private if permitted)
    params = {"limit": 1000, "exclude_archived": "true", "types": "public_channel"}
    try:
        ...
        if resp.status_code == 200:
            ...
            # If not found in public, try private if we have scope
            if data.get("error") == "missing_scope":
                logger.warning("slack_list_public_failed_missing_scope", needed="channels:read")
    except Exception as e:
        logger.warning("slack_list_channels_error", error=str(e))
    
    # 2. Try creating it if requested
    if create_if_missing:
        logger.info("slack_create_channel_attempt", name=clean_name)
        create_resp = await client.post(
            f"{_BASE_URL}/conversations.create", 
            headers=_headers(), 
            json={"name": clean_name}
        )
        create_data = create_resp.json()
        if create_data.get("ok"):
            chan_id = create_data["channel"]["id"]
            logger.info("slack_channel_created", name=clean_name, id=chan_id)
            return str(chan_id)
        else:
            err = create_data.get("error")
            logger.warning("slack_channel_create_failed", name=clean_name, error=err)
            if err == "missing_scope":
                logger.error("slack_insufficient_scopes", detail="Bot needs 'channels:write' or 'groups:write' to create channels.")
            # If error is name_taken, it might be private and not in list
            if err == "name_taken":
                return name_or_id
